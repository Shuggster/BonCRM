  ...
  async function handleCleanup() {
    try {
      ...
      if (data.deletedCount > 0) {
        toast(`Successfully removed ${data.deletedCount} duplicate teams.`)
        // Refresh the teams list
        await fetchTeams()
      } else {
        toast("No duplicate teams found.")
      }
    } catch (error) {
      toast(error instanceof Error ? error.message : "Failed to cleanup teams")
    } finally {
      setCleaning(false)
    }
  }
